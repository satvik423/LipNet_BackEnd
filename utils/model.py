from tensorflow.keras.layers import (
    Conv3D, ZeroPadding3D, MaxPooling3D,
    Dense, Activation, SpatialDropout3D, Flatten,
    Bidirectional, TimeDistributed, GRU, Lambda,
    BatchNormalization, Input
)
from tensorflow.keras.models import Model
from utils.layers import CTC
...
class LipNet(object):
    ...
    def build(self):
        ...
        self.model_pred = Model(inputs=self.input_data, outputs=self.y_pred)

    def summary(self):
        Model(inputs=self.input_data, outputs=self.y_pred).summary()

    def predict(self, input_batch):
        return self.model_pred.predict(input_batch)
