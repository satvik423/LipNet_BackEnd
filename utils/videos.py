...
import dlib
from utils.aligns import Align
...
class Video(object):
    ...
    def get_frames_mouth(self, detector, predictor, frames):
        ...
        for frame in frames:
            ...
            new_img_shape = (int(frame.shape[1] * normalize_ratio), int(frame.shape[0] * normalize_ratio))  # Note: width first, then height
            img = Image.fromarray(frame)
            resized_img = np.array(img.resize(new_img_shape, Image.BILINEAR))
